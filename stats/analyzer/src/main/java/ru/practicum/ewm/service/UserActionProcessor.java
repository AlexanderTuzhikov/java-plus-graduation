...
public class UserActionProcessor implements Runnable {
    ...
    @Override
    public void run() {
        Properties props = configService.getConsumerProperties("user-action");
        String topic = configService.getTopic("user-action");
        KafkaConsumer<String, UserActionAvro> consumer = new KafkaConsumer<>(props);
        ...
        try {
            ...
            while (true) {
                ConsumerRecords<String, UserActionAvro> records = consumer.poll(Duration.ofMillis(2000));
                if (records.isEmpty()) {
                    continue;
                }
                ...
                for (ConsumerRecord<String, UserActionAvro> record : records) {
                    try {
                        userActionService.updateAction(record);
                        offsets.put(
